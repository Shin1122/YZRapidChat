...
{
    //使用系统原生UITextView 或 UITextField
    if (![_yz_textView isKindOfClass:[YYTextView class]]) {
...
//        YYTextAttachment *attachment = [YYTextAttachment attachmentWithContent:note.object];
//        NSLog(@"%@",attachment.description);
//        
//        NSString *path = [[NSBundle mainBundle]pathForResource:[NSString stringWithFormat:@"%@",attachment.content] ofType:@"png" inDirectory:@"Emotion.bundle"];
//        
//        NSData *data = [NSData dataWithContentsOfFile:path];
//        YYImage *image = [YYImage imageWithData:data scale:2];
//        image.preloadAllAnimatedImageFrames = YES;
//        YYAnimatedImageView *imageView = [[YYAnimatedImageView alloc] initWithImage:image];
//        NSMutableAttributedString *attachText = [NSMutableAttributedString yy_attachmentStringWithContent:imageView contentMode:UIViewContentModeCenter attachmentSize:imageView.intrinsicContentSize alignToFont:[UIFont systemFontOfSize:16] alignment:YYTextVerticalAlignmentCenter];
...
{
    _textView  = textView;
    if (!([textView isKindOfClass:[UITextField class]] || [textView isKindOfClass:[UITextView class]]||[textView isKindOfClass:[YYTextView class]])) {
